...
u_init = 0;
K = (y_final - y_init) / (u_final - u_init);
...
t_debut_modele = t_reaction - dt; % -dt car il nous faut minimun 1 échantillon en plus pour détecter une montée
...
plot(t, u, 'g--', 'LineWidth', 1.5, 'DisplayName', 'Commande');
plot(t, y,       'b-',  'LineWidth', 1.5, 'DisplayName', 'Mesure');
plot(t, y_model, 'r--', 'LineWidth', 2.0, 'DisplayName', 'Modèle 1er ordre');

% Annotation tau (63%)
t_tau_abs = t_debut_modele + tau;          % correct time on model
y_tau_val = y_init + K * (u_final - u_init) * (1 - exp(-1));  % 63% of model

plot(t_tau_abs, y_tau_val, 'ms', 'MarkerSize', 10, 'LineWidth', 2, ...
    'DisplayName', sprintf('\\tau model = %.3f s (63%%)', tau));
grid on;
legend('Location', 'southeast', 'FontSize', 11);
xlabel('Temps (s)', 'FontSize', 12);
ylabel('Vitesse (rad/s)', 'FontSize', 12);
title(sprintf('Identification 1^{er} ordre joint1\nK=%.4f | \\tau=%.4f s', ...
    K, tau), 'FontSize', 13);
hold off;
